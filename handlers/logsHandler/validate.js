...
export function validatePublisher(newPublisher) {
    const PublisherRegistryFilePath = "logfiles/PublisherRegistry.json";
    filesystem.readFile(PublisherRegistryFilePath, function (err, data) {
        if (err) throw err;
        const PublisherRegistry = JSON.parse(data);
        
        if (newPublisher["publisher"] in PublisherRegistry) return false;

        return true;

        // PublisherRegistry["test"] = {
        //     "user": "xyz"
        // }
